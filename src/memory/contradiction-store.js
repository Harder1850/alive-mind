...
exports.recordContradiction = recordContradiction;
...
function contradictionKey(sk, ss, at) {
    return `${sk}:${ss}:${at}`;
}
function applyDecay(c) {
    const now = Date.now();
...
function recordContradiction(signal, failedActionType, sourceConfidence) {
    ...
    const c = { id: crypto.randomUUID(), signal_kind: signal.kind, signal_source: signal.source, failed_action_type: failedActionType, strength, initial_strength: strength, source_confidence: sourceConfidence, created_at: Date.now(), last_evaluated_at: Date.now(), failure_count: 1, resolved: false };
    contradictions.set(key, c);
    return c;
}
function evaluateSuppression(signal, actionType) {
    const votes = [];
    for (const [key, raw] of contradictions.entries()) {
        ...
        contradictions.set(key, c);
        if (c.strength < PRUNE_THRESHOLD) {
            contradictions.delete(key);
            continue;
        }
...
function resolveContradiction(id, reason) {
    for (const [key, c] of contradictions.entries()) {
        if (c.id === id) {
            contradictions.set(key, { ...c, resolved: true, resolution_reason: reason });
            return true;
        }
    }
    return false;
...
function decayAll() {
    for (const [key, c] of contradictions.entries()) {
        ...
        if (d.strength < PRUNE_THRESHOLD) {
            contradictions.delete(key);
        }
        else {
            contradictions.set(key, d);
        }
    }
}
function getActive() {
